...
const HomePage = () => {
  ...
  const getTotal = async () => {
    try {
      const { data } = await axios.get(
        `http://localhost:1012/api/v1/product/product-count`
      );
      setTotal(data?.total);
    } catch (error) {
      console.log(error);
  ...
  const loadMore = async () => {
    try {
      setLoading(true);
      const { data } = await axios.get(
        `http://localhost:1012/api/v1/product/product-list/${page}`
      );
      setLoading(false);
  ...
  const filterProduct = async () => {
    try {
      const { data } = await axios.post(
        `http://localhost:1012/api/v1/product/product-filter`,
        { checked }
      );
      setProducts(data?.products);
    } catch (error) {
          ...
          <div className="d-flex flex-wrap ">
            {products?.map((p) => (
              <div className="card m-2" key={p._id}>
                <img
                  src={`http://localhost:1012/api/v1/product/product-photo/${p._id}`}
